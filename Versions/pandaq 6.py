...
class EvalVisitor(pandaqVisitor):
	def visitRootAccio(self, ctx):
		[ctx_accio] = list(ctx.getChildren())
		self.visit(ctx_accio)
	...
	def visitSeleccionar(self, ctx):
		...
		if nom_taula in st.session_state:
			if campsSeleccionats == None: # seleccionar la taula sencera
				...
				taula = st.session_state[nom_taula][campsSeleccionats]
		else:
			if campsSeleccionats == None: # seleccionar la taula sencera
	...
	def visitSeleccionarIInnerJoin(self, ctx):
		...
		# df1.merge(df2)
		# pd.merge(df1, df2)
		for nom_taula2, columna1, columna2 in innerjoin: # Columna 1 = Columna 2
			taula2 = pd.DataFrame()
			if nom_taula2 in st.session_state:
				taula2 = st.session_state[nom_taula2]
			else:
				taula2 = taules[nom_taula2]
			taula = taula.merge(taula2)
		# camps seleccionats
		taula = taula[campsSeleccionats]
		# Visualitzar taula
		with pagina:
			st.table(taula)
		return taula
